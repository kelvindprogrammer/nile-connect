...
func devLogin(w http.ResponseWriter, r *http.Request) {
	// This endpoint mints a real, signed nile_session for an arbitrary role.
	// Hiding the buttons behind a hostname check in Login.tsx only hides the
	// buttons — the route is in vercel.json and is reachable on the public
	// deployment, where GET /api/auth/dev-login?role=staff would hand any
	// visitor a staff session and with it moderation, student records and
	// employer approval. It must therefore refuse to run anywhere but a local
	// host, matching the 404-when-not-configured posture of the other dev
	// endpoints.
	if !mw.IsLocalRequest(r) {
		respond.Error(w, http.StatusNotFound, "not found")
		return
	}
