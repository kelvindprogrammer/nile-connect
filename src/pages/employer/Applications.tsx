                ...
                {FILTERS.map(f => (
                    <button key={f} onClick={() => setFilter(f)}
                        className={`flex items-center gap-1.5 px-3 md:px-5 py-2 rounded-xl font-semibold text-[8px] transition-all whitespace-nowrap flex-shrink-0
                            ${filter === f ? 'bg-black text-white shadow-green' : 'text-black/40 hover:text-black'}`}>
                        {f.toUpperCase()}
                        <span className={`text-[7px] px-1.5 py-0.5 rounded-full font-semibold ${filter === f ? 'bg-white/20 text-white' : 'bg-black/5 text-black/40'}`}>
                            {countFor(f)}
                        </span>
                    </button>
                ))}
                ...
                <div className="space-y-3">
                    {filtered.map(app => (
                        <div key={app.id} className="bg-white border border-gray-100 rounded-[20px] p-4 md:p-5 flex flex-col sm:flex-row items-start sm:items-center gap-4 hover:shadow-card transition-all">
                            <div className="w-10 h-10 bg-nile-blue text-white rounded-xl flex items-center justify-center font-semibold text-sm flex-shrink-0">
                                {(app.student_name || '?').charAt(0).toUpperCase()}
                            </div>
                            <div className="flex-1 min-w-0">
                                <div className="flex flex-wrap items-center gap-2 mb-1">
                                    <p className="font-semibold text-sm text-black truncate">{app.student_name}</p>
                                    {app.is_verified && (
...
const StatusSelect = ({ current, loading, onChange }: {
    ...
    return loading ? (
        <div className={`flex items-center gap-2 px-3 py-2 border-[2px] rounded-xl font-semibold text-[8px] ${cfg.color}`}>
            <Loader2 size={11} className="animate-spin" /> ...
        </div>
    ) : (
        <div className="relative">
            <select value={current} onChange={e => onChange(e.target.value)}
                className={`appearance-none pl-3 pr-8 py-2 border-[2px] rounded-xl font-semibold text-[8px] tracking-wider cursor-pointer outline-none ${cfg.color} border-current`}>
