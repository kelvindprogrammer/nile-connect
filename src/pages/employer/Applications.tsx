...
const EmployerApplications = () => {
    ...
    if (loading && applications.length === 0) return (
        <div className="p-4 md:p-8 space-y-6 animate-pulse">
            ...
            <div className="flex gap-2">{[1,2,3,4].map(i => <div key={i} className="h-9 bg-paper-100 rounded-xl w-24" />)}</div>
            {[1,2,3,4].map(i => <div key={i} className="h-24 bg-paper-100 rounded-[20px]" />)}
        </div>
    );
    ...
    return (
        <div className="p-4 md:p-8 space-y-8 anime-fade-in font-sans pb-20 text-left min-h-full">
            <div className="border-b border-paper-300 pb-6">
                <h2 className="co-display text-3xl md:text-4xl text-ink-800 leading-none">Applications</h2>
            ...
            <div className="flex bg-white p-1 border border-paper-300 rounded-xl shadow-sm overflow-x-auto">
                <button onClick={() => setFilter('all')}
                    className={`flex items-center gap-1.5 px-3 md:px-5 py-2 rounded-xl font-semibold text-[8px] transition-all whitespace-nowrap flex-shrink-0
                        ${filter === 'all' ? 'bg-ink-900 text-white shadow-green' : 'text-paper-700 hover:text-ink-800'}`}>
                    ALL
                    <span className={`text-[7px] px-1.5 py-0.5 rounded-full font-semibold ${filter === 'all' ? 'bg-white/20 text-white' : 'bg-paper-100 text-paper-700'}`}>
                        {countFor('all')}
                    </span>
                </button>
                {APPLICATION_STAGES.map(s => (
                    <button key={s.value} onClick={() => setFilter(s.value)}
                        className={`flex items-center gap-1.5 px-3 md:px-5 py-2 rounded-xl font-semibold text-[8px] transition-all whitespace-nowrap flex-shrink-0
                            ${filter === s.value ? 'bg-ink-900 text-white shadow-green' : 'text-paper-700 hover:text-ink-800'}`}>
                        {s.label.toUpperCase()}
                        <span className={`text-[7px] px-1.5 py-0.5 rounded-full font-semibold ${filter === s.value ? 'bg-white/20 text-white' : 'bg-paper-100 text-paper-700'}`}>
                            {countFor(s.value)}
                        </span>
                    </button>
                ))}
            ...
            <div className="flex flex-col md:flex-row gap-3">
                ...
                <div className="relative">
                    <ArrowUpDown size={12} className="absolute left-4 top-1/2 -translate-y-1/2 text-paper-600 pointer-events-none" />
                    <select
                        value={sort}
                        ...
                        <div key={app.id} className="bg-white border border-paper-300 rounded-[20px] p-4 md:p-5 flex flex-col sm:flex-row items-start sm:items-center gap-4 hover:shadow-card transition-all">
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex flex-wrap items-center gap-2 mb-1">
                                    <button onClick={() => navigate(`/employer/candidates/${app.id}`)} className="font-semibold text-sm text-ink-800 truncate hover:underline">{app.student_name}</button>
                                    {app.is_verified && (
                                ...
                                <div className="flex flex-wrap gap-3 mt-1">
                                    {app.major && (
                                        <span className="flex items-center gap-1 text-[7px] font-semibold text-paper-700">
                                            <GraduationCap size={9} />{app.major}
                                        </span>
                                    )}
                                    {app.graduation_year > 0 && (
                                        <span className="text-[7px] font-semibold text-paper-600">CLASS OF {app.graduation_year}</span>
                                    )}
                                    {app.gpa > 0 && (
                                        <span className="text-[7px] font-semibold text-paper-600">GPA {app.gpa.toFixed(2)}</span>
                                    )}
                                    {app.applied_at && (
                                        <span className="text-[7px] font-semibold text-paper-600">
                            ...
                            <div className="flex items-center gap-2 w-full sm:w-auto flex-shrink-0">
                                <button onClick={() => navigate('/employer/messages')}
                                    className="p-2 border border-paper-300/10 rounded-xl text-paper-700 hover:border-paper-400 hover:text-nile-blue transition-all">
