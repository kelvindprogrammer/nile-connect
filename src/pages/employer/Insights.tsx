    ...
    if (loading) return (
        <div className="p-4 md:p-6 space-y-4 animate-pulse max-w-4xl mx-auto">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[1,2,3,4].map(i => <div key={i} className="h-24 bg-paper-200 rounded-xl" />)}
            </div>
    ...
    return (
        <div className="p-4 md:p-6 pb-24 md:pb-8 space-y-6 anime-fade-in font-sans max-w-4xl mx-auto">
            ...
            <Card title="Latest applicants" action={
                <button onClick={() => navigate('/employer/applications')} className="text-xs font-medium text-nile-blue flex items-center gap-1 hover:text-ink-800">
                    View all <ChevronRight size={13} />
                </button>
            }>
                {applications.length === 0 ? (
                    <div className="py-10 text-center border-2 border-dashed border-paper-300 rounded-xl">
                        <Users size={24} className="text-paper-400 mx-auto mb-3" />
                        <p className="text-sm text-paper-600">No applications yet — post a job to get started</p>
                    </div>
                ) : (
                            ...
                            <div key={app.id} className="flex items-center gap-3 py-3">
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2 flex-wrap">
                                        <p className="font-medium text-sm text-ink-800 truncate">{app.student_name}</p>
                                        {app.is_verified && <CheckCircle2 size={13} className="text-nile-green flex-shrink-0" strokeWidth={2.5} />}
                                    </div>
                                    <p className="text-xs text-paper-600 truncate">{app.job_title} · {app.major || '—'}</p>
                                </div>
                                <span className={`text-[11px] font-medium px-2 py-1 rounded-full flex-shrink-0 ${statusColor[app.status] || 'bg-paper-100 text-paper-600'}`}>
                                    {app.status}
                                </span>
                            </div>
                        ))}
            ...
            <Card title="Your listings" action={
                <button onClick={() => navigate('/employer/jobs')} className="text-xs font-medium text-nile-blue flex items-center gap-1 hover:text-ink-800">
                    Manage <ChevronRight size={13} />
                </button>
            }>
                {jobs.length === 0 ? (
                    <div className="py-10 text-center border-2 border-dashed border-paper-300 rounded-xl">
                        <Briefcase size={24} className="text-paper-400 mx-auto mb-3" />
                        <p className="text-sm text-paper-600 mb-3">No listings yet</p>
                        <button onClick={() => navigate('/employer/jobs')}
                            className="px-4 py-2 bg-ink-900 text-white rounded-xl font-medium text-xs">
                    ...
                    <div className="space-y-2">
                        {jobs.slice(0, 5).map(job => (
                            <div key={job.id} onClick={() => navigate('/employer/jobs')}
                                className="flex items-center justify-between p-3 border border-paper-300 rounded-xl hover:bg-paper-100 transition-all cursor-pointer group">
                                <div className="flex-1 min-w-0">
                                    <p className="font-medium text-sm text-ink-800 truncate leading-none group-hover:text-nile-blue transition-colors">{job.title}</p>
                                    <div className="flex items-center gap-2 mt-1">
                                        <span className={`text-[11px] font-medium px-1.5 py-0.5 rounded-full ${statusColor[job.status] || 'bg-paper-100 text-paper-600'}`}>{job.status}</span>
                                        <span className="text-[11px] text-paper-600">{job.applicant_count} applicants</span>
                                    </div>
                                </div>
                                <ArrowRight size={14} className="text-paper-500 group-hover:text-ink-800 transition-colors flex-shrink-0 ml-2" />
                            </div>
                        ))}
                    </div>
                )}
