...
const StaffProfile = () => {
    ...
    return (
        <div className="p-4 md:p-8 space-y-6 anime-fade-in font-sans pb-24 text-left max-w-4xl mx-auto">
            ...
            {/* Hero */}
            <div className="bg-white border border-paper-300 rounded-[24px] shadow-card overflow-hidden">
                ...
                <div className="px-5 md:px-8 pb-6 relative">
                    <div className="absolute -top-10 left-5 md:left-8 w-20 h-20 rounded-xl border border-paper-300 bg-white shadow-card overflow-hidden">
                    ...
                    <Card title="Focus areas">
                        <div className="flex flex-wrap gap-2">
                            {FOCUS_AREAS.map(area => (
                                <span key={area} className="px-3 py-1.5 bg-paper-200 text-paper-700 rounded-full text-xs font-medium">
                                    {area}
                                </span>
                            ))}
                        </div>
                    ...
                    <Card title="Platform activity" subtitle="Platform-wide totals, not individually attributed">
                        {loading ? (
                            <div className="grid grid-cols-2 gap-4 animate-pulse">
                                {[1, 2, 3, 4].map(i => <div key={i} className="h-20 bg-paper-200 rounded-xl" />)}
                            </div>
                        ) : stats ? (
                            <div className="grid grid-cols-2 gap-4">
                                {[
                                    ...
                                    { label: 'Applications', v: stats.total_applications, sub: 'Total submitted', c: 'bg-paper-100 border-paper-300', cv: 'text-ink-700' },
                                ].map(s => (
                                    <div key={s.label} className={`border rounded-xl p-4 ${s.c}`}>
                                        <p className={`text-2xl font-semibold leading-none ${s.cv}`}>{s.v.toLocaleString()}</p>
                                        <p className="text-xs font-medium text-paper-700 mt-1">{s.label}</p>
                                        <p className="text-xs text-paper-600 mt-0.5">{s.sub}</p>
                                    </div>
                                ))}
                            </div>
                        ) : (
                            <p className="text-sm text-paper-600 text-center py-8">Failed to load stats</p>
                        )}
                    </Card>
                </div>
                        ...
                        <div className="space-y-3">
                            {[
                                ...
                                <div key={r.label} className="flex items-start gap-3">
                                    <div className="w-8 h-8 bg-nile-white border border-paper-300 rounded-lg flex items-center justify-center text-nile-blue flex-shrink-0">{r.icon}</div>
                                    <div className="min-w-0">
                                        <p className="text-xs text-paper-600">{r.label}</p>
                                        <p className="text-sm font-medium text-ink-800 truncate">{r.value}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    <Card title="Connect">
                        <div className="space-y-2">
                            <a
                                ...
                                className="flex items-center gap-3 p-3 border border-paper-300 rounded-xl hover:bg-nile-blue hover:text-white transition-all group"
                            >
                            ...
                            <a
                                href="/staff/messages"
                                className="flex items-center gap-3 p-3 border border-paper-300 rounded-xl hover:bg-nile-blue hover:text-white transition-all"
                            >
