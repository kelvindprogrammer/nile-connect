...
export const getConversations = async (): Promise<Conversation[]> => {
    const { data } = await apiClient.get<Envelope<{ conversations: Conversation[] }>>('/messages/conversations');
...
export const searchUsers = async (q: string, role?: string): Promise<UserProfile[]> => {
    ...
    const { data } = await apiClient.get<Envelope<{ users: UserProfile[] }>>('/users/search', { params });
