    ...
    return (
        <div className="bg-white border border-paper-300 rounded-xl shadow-card overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-paper-200">
                ...
                {seeAllTo && (
                    <button onClick={() => navigate(seeAllTo)} className="text-xs font-medium text-paper-600 hover:text-nile-blue transition-colors">
                        See all
                    </button>
                )}
            </div>
            {isLoading ? (
                ...
                <p className="px-4 py-6 text-xs text-paper-600 text-center">{emptyLabel}</p>
            ) : (
