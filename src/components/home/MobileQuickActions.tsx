        ...
        <div className="space-y-4">
            {(greeting || name) && (
                <div>
                    ...
                    {name && <h1 className="text-lg font-semibold text-gray-900 leading-tight">{name}</h1>}
                ...
                <div className="grid grid-cols-3 gap-2">
                    {stats.map(s => (
                        <button
                            ...
                            disabled={!s.to}
                            className="bg-white border border-gray-100 rounded-2xl shadow-card px-3 py-2.5 text-left disabled:cursor-default enabled:hover:border-gray-200 transition-colors"
                        >
            ...
            {actions.length > 0 && (
                <div className="bg-white border border-gray-100 rounded-2xl shadow-card p-2">
                    <div className="grid grid-cols-4 gap-1">
                        {actions.map(({ label, icon: Icon, to, badge }) => (
                            <button
                                key={to}
                                onClick={() => navigate(to)}
                                className="relative flex flex-col items-center gap-1.5 py-2.5 px-1 rounded-xl hover:bg-gray-50 active:scale-[0.97] transition-all"
                            >
