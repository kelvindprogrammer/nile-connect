...
const EventsCard: React.FC<{ seeAllTo: string }> = ({ seeAllTo }) => {
    ...
    return (
        <SidebarCard title="Upcoming events" seeAllTo={seeAllTo} isLoading={events === null} empty={events?.length === 0} emptyLabel="No upcoming events">
            {events?.map(ev => (
                <div key={ev.id} className="flex items-start gap-3 px-4 py-2.5 hover:bg-paper-100 transition-colors">
                    ...
                    <div className="min-w-0">
                        <p className="text-xs font-medium text-ink-800 truncate">{ev.title}</p>
                        <p className="text-[11px] text-paper-600 mt-0.5 truncate">
