    ...
    return (
        ...
        >
            ...
            <span className={`
                ...
                font-black tracking-[0.1em] mt-1.2 transition-colors
...
const DashboardLayout: React.FC<DashboardLayoutProps> = ({ children }) => {
    ...
    return (
        <div className="flex h-screen bg-nile-white text-black font-sans overflow-hidden">
            ...
            {/* Compact Desktop Sidebar - Hidden on Mobile */}
            ...
            <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t-[2px] border-black h-16 px-4 flex items-center justify-between z-[40] shadow-[0_-4px_10px_rgba(0,0,0,0.05)]">
                <SlimNavItem to="/student" icon={<Home />} label="HOME" isActive={location.pathname === '/student'} mobile />
                <SlimNavItem to="/student/feed" icon={<MessageSquare />} label="FEED" isActive={location.pathname === '/student/feed'} mobile />
                <div onClick={() => navigate('/student/feed')} className="flex-1 flex flex-col items-center justify-center -mt-8">
                     <div className="w-12 h-12 bg-nile-green border-2 border-black rounded-full flex items-center justify-center shadow-[4px_4px_0px_0px_#000] rotate-45 hover:rotate-0 transition-transform cursor-pointer">
                        <ChevronRight className="-rotate-45 text-white" />
                     </div>
                </div>
                <SlimNavItem to="/student/jobs" icon={<Briefcase />} label="JOBS" isActive={location.pathname === '/student/jobs'} mobile />
                <SlimNavItem to="/student/profile" icon={<User />} label="ME" isActive={location.pathname === '/student/profile'} mobile />
            </nav>
            ...
            <main className="flex-1 flex flex-col min-w-0 bg-white h-full relative">
                <header className="h-14 border-b-[2px] border-black flex items-center justify-between px-4 md:px-6 bg-white/80 backdrop-blur-md sticky top-0 z-20 flex-shrink-0">
                    ...
                    <div className="flex-1 mx-3 md:mx-6 flex items-center justify-center max-w-xl">
                        <div className="relative group w-full">
                            ...
                            <input 
                                type="text" 
                                placeholder="SEARCH..." 
                                className="w-full bg-nile-white border-[2px] border-black rounded-lg py-1.5 pl-10 pr-3 font-black text-[9px] uppercase outline-none focus:shadow-[2px_2px_0px_0px_#1E499D] transition-all"
                            />
                        </div>
                    </div>
                    ...
                    <div className="flex items-center space-x-2 md:space-x-4">
                        ...
                        <div className="hidden md:block h-5 w-[1px] bg-black/5 mx-2" />

                        <div className="hidden md:flex items-center space-x-2.5 cursor-pointer group" onClick={() => navigate('/student/profile')}>
